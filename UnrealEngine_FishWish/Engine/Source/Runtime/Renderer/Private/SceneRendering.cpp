...
static TAutoConsoleVariable<int32> CVarEnableLateLatchingDebug(
	TEXT("r.EnableLateLatchingDebugMode"),
	0,
	TEXT("Whether to allow late-latching DebugMode."),
	ECVF_RenderThreadSafe | ECVF_Scalability);
// END META SECTION - PORT PICO LateLatching
/*-----------------------------------------------------------------------------
	...
	FViewUniformShaderParameters& ViewUniformShaderParameters) const
{
	...
	// BEGIN META SECTION - PORT PICO LateLatching
	// Late Latching Debug Color
	ViewUniformShaderParameters.DebugColor = FVector3f(1,1,1);
	// END META SECTION - PORT PICO LateLatching
...
void FViewInfo::UpdateLateLatchData()
{
	...
	{
		//Late Latching Debug Color
		static const auto CVar = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.EnableLateLatchingDebugMode"));
		if (CVar->GetValueOnAnyThread())
		{
			CachedViewUniformShaderParameters->DebugColor=FVector3f(1,0,0);
		}
