...
void UGameViewportClient::ReceivedFocus(FViewport* InViewport)
{
	...
	if (GEngine)
	{
		...
		// make sure PIEs are fully initialized before switching primary PIE
		if (NumClients > 1 && 
			PlayInSettings->IsSetActivePIEToPrimary() &&
			RunUnderOneProcess)
		{
			UEditorEngine* EditorEngine = Cast<UEditorEngine>(GEngine);
			if (EditorEngine &&
				EditorEngine->GetPlayInEditorSessionInfo()->NumClientInstancesCreated == NumClients)
			{
				EditorEngine->SwitchPrimaryPIE(this);
			}
		}
	}
#endif
