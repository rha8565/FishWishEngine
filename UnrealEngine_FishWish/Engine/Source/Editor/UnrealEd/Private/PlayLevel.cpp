...
// BEGIN META SECTION - VR MultiPlayer
void UEditorEngine::SwitchPrimaryPIE(UGameViewportClient* InViewport)
{
	SwitchPrimaryPIE(InViewport, TOptional<int>());
}

void UEditorEngine::SwitchPrimaryPIE(int PrimaryPIEIndex)
{
	SwitchPrimaryPIE(nullptr, PrimaryPIEIndex);
}

void UEditorEngine::SwitchPrimaryPIE(UGameViewportClient* InViewport, TOptional<int> PrimaryPIEIndex)
{
	if (InViewport && PrimaryPIEIndex.IsSet())
	{
		UE_LOG(LogPlayLevel, Warning, TEXT("InViewport and PrimaryPIEIndex should not be set at the same time."));
		return;
	}
	...
	{
		UE_LOG(LogPlayLevel, Warning, TEXT("Failed to switch primary PIE."));
	}
}

void UEditorEngine::SwitchPrimaryPIE(FWorldContext* OldPrimaryWorld, FWorldContext* NewPrimaryWorld)
{
	if (!(GEngine &&
	...
	if (NewPrimaryWorld->GameViewport && NewPrimaryWorld->GameViewport->GetGameViewport() &&
		OldPrimaryWorld->GameViewport && OldPrimaryWorld->GameViewport->GetGameViewport())
	{
		TogglePrimaryWorld(NewPrimaryWorld, true);
		TogglePrimaryWorld(OldPrimaryWorld, false);

		XRSystem->SwitchPrimaryPIE(NewPrimaryWorld->PIEInstance);
	}
}
// END META SECTION - VR MultiPlayer
