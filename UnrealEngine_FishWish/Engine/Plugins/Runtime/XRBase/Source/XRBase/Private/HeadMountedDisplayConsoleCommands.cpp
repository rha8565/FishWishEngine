...
static void SwitchPrimaryPIE(const TArray<FString>& Args, UWorld*, FOutputDevice& Ar)
{
	...
	if (!GetDefault<ULevelEditorPlaySettings>()->IsSetCVarPIEToPrimary())
	{
		Ar.Logf(TEXT("Please enable \"Set CVar PIE To Primary\" in Editor Preference!"));
		return;
	}

	if (Args.Num())
	{
		GEngine->SwitchPrimaryPIE(FCString::Atoi(*Args[0]));
	}
	else
	{
...
static FAutoConsoleCommand CSwitchHMDCmd(
	...
	*LOCTEXT("CCommandText_SetPrimaryPIEIndex", "Set primary PIE index on the fly.").ToString(),
